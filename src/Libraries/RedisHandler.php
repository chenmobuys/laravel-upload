...
class RedisHandler
{
    ...
    public static function hashExists($fileHash)
    {
        ...
        if ( class_exists('\Predis\Connection\ConnectionException') ) {
            try {
                $result = Redis::hexists(config('chenupload.REDIS_KEY'), $fileHash);
    ...
    public static function getFilePathByHash($fileHash)
    {
        ...
        if ( class_exists('\Predis\Connection\ConnectionException') ) {
            try {
                $filePath = Redis::hget(config('chenupload.REDIS_KEY'), $fileHash);
            } catch ( ConnectionException $e ) {
    ...
    public static function setOneHash($fileHash, $savedPath)
    {
        ...
        if(class_exists('\Predis\Connection\ConnectionException')) {
            try {
                $result = Redis::hset(config('chenupload.REDIS_KEY'), $fileHash, $savedPath);
            } catch ( ConnectionException $e ) {
    ...
    public static function build()
    {
        ...
        $groupNames = array_keys(config('chenupload.GROUPS'));
        $uploadPath = config('chenupload.UPLOAD_PATH');
    ...
    public static function deleteOneHash($fileHash)
    {
        ...
        if(class_exists('\Predis\Connection\ConnectionException')) {
            try {
                $result = Redis::hdel(config('chenupload.REDIS_KEY'), $fileHash);
            } catch ( ConnectionException $e ) {
    ...
    public static function deleteAllHashes()
    {
        ...
        if(class_exists('\Predis\Connection\ConnectionException')) {
            try {
                $result = Redis::del(config('chenupload.REDIS_KEY'));
            } catch ( ConnectionException $e ) {
