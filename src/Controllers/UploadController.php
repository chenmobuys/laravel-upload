...
namespace Chenmobuys\LaravelUpload\Controllers;


use Illuminate\Routing\Controller;
use Chenmobuys\LaravelUpload\Libraries\Receiver;
use Chenmobuys\LaravelUpload\Libraries\ConfigMapper;
use Chenmobuys\LaravelUpload\Libraries\Responser;
use Chenmobuys\LaravelUpload\Libraries\RedisHandler;
...
class UploadController extends Controller
{
    ...
    public function preprocess()
    {
        ...
        if (!($fileName && $fileSize)) {
            return Responser::reportError('invalid_file_params');
        }
...
//        }
    ...
    public function saveChunk()
    {
        ...
        if (!($this->receiver->chunkTotalCount && $this->receiver->chunkIndex && $this->receiver->uploadExt && $this->receiver->uploadBaseName && $subDir)) {
            return Responser::reportError('invalid_chunk_params', true, $this->receiver->uploadHead, $this->receiver->uploadPartialFile);
        }
        # 防止被人为跳过验证过程直接调用保存方法，从而上传恶意文件
        if (!is_file($this->receiver->uploadPartialFile)) {
            return Responser::reportError('invalid_operation', true, $this->receiver->uploadHead, $this->receiver->uploadPartialFile);
        }
        ...
        if (!$this->receiver->file->isValid()) {
            return Responser::reportError('http_post_only', true, $this->receiver->uploadHead, $this->receiver->uploadPartialFile);
        }
        ...
        if ($this->receiver->chunkIndex === $this->receiver->chunkTotalCount) {
            ...
            if (!($result['savedPath'] = $this->receiver->renameTempFile())) {
                return Responser::reportError('rename_file_fail', true, $this->receiver->uploadHead, $this->receiver->uploadPartialFile);
            }
    ...
    public function uploadCancel()
    {
        ...
        if(!(@unlink($this->receiver->uploadHead) && @unlink($this->receiver->uploadPartialFile))){
            return 'delete_file_fail';
        }

        return Responser::returnResult(['error'=>0]);
    }
